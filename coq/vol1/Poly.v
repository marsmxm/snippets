...
Fixpoint split {X Y : Type} (l : list (X*Y))
  ...
  match l with
  | nil => (nil, nil)
  | p :: l' => let split_l' := split l' in
             ((fst p) :: (fst split_l'), (snd p) :: (snd split_l'))
  end.
...
Proof.
  intros. induction l as [|x l'].
  - reflexivity.
  - simpl. rewrite -> map_assoc. rewrite -> IHl'. reflexivity. Qed.
(** [] *)
 ...
 *)
Theorem nth_error_n : forall (X : Type) (n : nat) (l : list X),
...
Fixpoint exp_cnat_nat (n : cnat) (m : nat) : cnat :=
  match m with
  | O => one
  | (S m') => mult n (exp_cnat_nat n m')
  end.

Definition exp (n m : cnat) : cnat := exp_cnat_nat n (m nat (fun y => (S y)) 0).
