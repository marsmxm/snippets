                      ...
                      unsigned char* const blueChannel)
{
  ...
  uchar4 imgRGB = inputImageRGBA[thread_1D_pos];
  redChannel[thread_1D_pos] = imgRGB.x;
  greenChannel[thread_1D_pos] = imgRGB.y;
  blueChannel[thread_1D_pos] = imgRGB.z;
}
                                ...
                                const float* const h_filter, const size_t filterWidth)
{
  ...
  size_t filterBytes = sizeof(float) * filterWidth * filterWidth;
                        ...
                        const int filterWidth)
{
  ...
  const dim3 gridSize(numCols / 32 + (numCols % 32 == 0 ? 0 : 1), numRows / 32 + (numRows % 32 == 0 ? 0 : 1));
