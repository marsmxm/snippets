...
2H	LDBU	current,k,base		A	&Get next element of input.\lineref|1.3.3KA1|
	...
	PBNZ	t,1F			A\bg{B}	&\lineref|1.3.3KC1|
	ORL	current,#80		B	&If so, tag it.\lineref|1.3.3open|
	STBU	current,k,base		B	&\lineref|1.3.3tag1|
0H	ADD	k,k,1			B	&\lineref|1.3.3size2|
	LDBU	start,k,base		B	&Put the next nonformat\lineref|1.3.3freq|
	BZ	start,0B		B\bg{0}	&\qquad input symbol in \.START..\lineref|1.3.3never|\lineref|1.3.3KC2|
1H	CMP	t,current,')'		C	&Is it `{\tt )}'?\lineref|1.3.3KC3|
	PBNZ	t,0F			C\bg{D}	&
	ORL	start,#80		D	&\lineref|1.3.3close|
	STBU	start,k,base		D	&Replace `{\tt )}' by tagged \.START..\lineref|1.3.3tag2|
0H	ADD	k,k,1			C	&\lineref|1.3.3size3|
...
%%%rt  1.3.3proga.rt & (6+6A+7B+4C+4D+E+2F+4G+5H+8J+3Q+6K+4P+9R)\upsilon
%%%rt  1.3.3proga.mem & (2A+2B+1D+1F+2H+2J+1Q+1K+2R)\mu
%%%off