...
%	File:	5.2.1/mlistishift.mms
%	Author:	Martin Ruckert
%	Title:	Program M, Multiple list insertion using shifting
...
%%%rton
:Sort		SET 	i,1<<(d+3)		 1 & $i\is M$. 
		JMP	1F			 1 &
0H		STCO	0,head,i		 M & Clear heads.\lineref|5.2.1-mlisti-init|
...
%%%
		...
		JMP	0F			 1 &
%%%
M2		LDO	k,key,j			 N & \otherfoolit{q} \foolit{q}\ut  M2. Set up \setotherfool, \setfool, $K$.\\ $K\is K_j$.
		SRU	i,k,e-3-d			 N & $i\is {\lfloor M\cdot K_j/2^{e}\rfloor}$.\lineref|5.2.1-mlisti-sru|
...
%%%on
%%%rt  5.2.1mlistishift.rt	& (6B+15N-3A+3M+13)\upsilon + (2B+5N-A+M)\mu
%%%off
		POP	1,0
%%%
		PREFIX	:
