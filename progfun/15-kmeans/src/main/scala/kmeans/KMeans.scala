...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val builder = GenMap.newBuilder[Point, GenSeq[Point]]
    means.foreach(mean => builder += ((mean, GenSeq[Point]())))
    builder.result() ++ points.groupBy(p => findClosest(p, means))
  }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] =
    oldMeans.map(old => findAverage(old, classified.getOrElse(old, Nil)))

  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean =
    oldMeans.forall(old => {
      val newMean = newMeans(oldMeans.indexOf(old))
      old.squareDistance(newMean) <= eta
    })
