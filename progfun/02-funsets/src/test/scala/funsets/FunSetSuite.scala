...
class FunSetSuite extends FunSuite {
  ...
  test("intersect") {
    new TestSets {
      ...
      assert(!contains(s, 2))
      assert(!contains(s, 3))
      s = intersect(union(s1, s2), union(s2, s3))
      assert(!contains(s, 1))
      assert(contains(s, 2))
      assert(!contains(s, 3))
    }
  ...
  test("diff") {
    new TestSets {
      var s = diff(s1, s2);
      println("diff(s1,s2) = " + FunSets.toString(s))
      assert(contains(s, 1))
      assert(!contains(s, 2))
      assert(!contains(s, 3))
      s = diff(union(s1, s2), union(s2, s3))
      assert(contains(s, 1))
      assert(!contains(s, 2))
      assert(!contains(s, 3))
    }
